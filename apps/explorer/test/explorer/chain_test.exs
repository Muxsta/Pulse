defmodule Explorer.ChainTest do
  ...
  describe "extract_db_username/1" do
    test "extracts correct db username" do
      db_url = "postgresql://viktor:@localhost:5432/blockscout-dev-1"
      assert Chain.extract_db_username(db_url) == "viktor"
    end

    test "returns empty db username" do
      db_url = ""
      assert Chain.extract_db_username(db_url) == ""
    end

    test "returns nil db username" do
      db_url = nil
      assert Chain.extract_db_username(db_url) == ""
    end
  end

  describe "extract_db_password/1" do
    test "extracts correct db password" do
      db_url = "postgresql://viktor:pass1@localhost:5432/blockscout-dev-1"
      assert Chain.extract_db_password(db_url) == "pass1"
    end

    test "returns empty db password" do
      db_url = ""
      assert Chain.extract_db_password(db_url) == ""
    end

    test "returns nil db password" do
      db_url = nil
      assert Chain.extract_db_password(db_url) == ""
    end
  end
